...
func TestParseMarshalAndBack(t *testing.T) {
	mod, err := LoadCLVMFromStrings(zap.NewNop(), "main.clvm", map[string]string{"main.clvm": file, "condition_codes.clvm": ConditionCodes})
	...
	var out interface{}
	err = json.Unmarshal(med, &out)
	require.NoError(t, err)
}

const file = `(mod (MOD_HASH MY_PUBKEY RECOVERY_DID_LIST_HASH NUM_VERIFICATIONS_REQUIRED mode amount message my_id my_puzhash parent_innerpuzhash_amounts_for_recovery_ids pubkey recovery_list_reveal)
